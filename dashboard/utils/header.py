...
[style*="color:var(--ua-ink-dim-2)" i], [style*="color:var(--ua-ink-dim)" i] {
    /* Was a flat #7F899A, which measured 3.26:1 on the light page background --
       this global override was itself emitting a sub-AA colour. Its three
       sibling rules above already use theme-aware tokens; --ua-text-cap is the
       dim tier and measures 6.59:1 dark / 5.34:1 light, so use it rather than
       any fixed hex (a hex cannot satisfy both themes: #68707E clears light at
       4.60:1 and fails dark at 3.89:1). */
    color: var(--ua-text-cap) !important;
}
...
/* ── Semantic ink variables (GENERATED) ───────────────────────────────────
   Tinted pills (category chips, confidence badges, status text, the regime
   strip) emit `color: var(--ua-ink-...)` rather than a hex, because the theme
   is resolved in the BROWSER -- ?theme= or localStorage['ua-theme'], see
   scripts/inject_boot_splash -- so the server cannot know which derivation to
   bake in. Remapping an emitted hex with `[style*=";color:#HEX"]` is not an
   option either: Streamlit's sanitizer re-serializes inline styles, so
   `color:#9B69F1` arrives as `color: rgb(155, 105, 241)` and the attribute
   selector never matches. A custom property survives untouched and lets CSS,
   the only layer that sees data-ua-theme, choose. Values are generated by
...
# The generated block is substituted once at import time. Kept out of the
# literal itself so the CSS above stays greppable and the derivation stays in
# utils/theme with its tests.
try:
    from utils.theme import ink_variables_css as _ua_ink_vars, semantic_ink_entries as _ua_ink_entries
    _CSS = _CSS.replace("__UA_LIGHT_INK_OVERRIDES__", _ua_ink_vars(_ua_ink_entries()))
except Exception:
    _CSS = _CSS.replace("__UA_LIGHT_INK_OVERRIDES__", "")
